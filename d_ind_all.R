...
d_ind_all_2012 <- read.dta13(file.path(working, 'descriptive_all_countries_macsec_szclass_all_2012.dta')) %>%  as_tibble() %>% 
  select(country, 
         mac_sector,    # NACE2 industry
         szclass,       # size class
         year, 
...
add_info <- anti_join(d_ind_all_2012, d_ind_all, c('country', 'mac_sector', 'szclass', 'year'))

# stack the most recent dataset, adding the additional
# info from the previous release
d_ind_all <- rbind(d_ind_all, add_info)

# sorting
d_ind_all <- arrange(d_ind_all, country, mac_sector, szclass, year)
