...
plot_avg_size <- ggplot(data=full_descr %>% group_by(year) %>%
...
plot_relshare_size <- ggplot(data=full_descr, aes(x=year, y=size_rel_share, colour=as.factor(szclass)))+
...
plot_relshare_emp <- ggplot(data=full_descr, aes(x=year, y=rel_emp_share, colour=as.factor(szclass)))+
  geom_line(size=1)+facet_wrap(~country, scales='free_y')+theme_bw()+labs(colour='Size class')+theme(legend.position=c(.68,.07))+
...
plot_lc_l_norm <- ggplot(data=full_descr, aes(x=year, y=lc_l_mean_norm, colour=as.factor(szclass)))+
  geom_line(size=1)+facet_wrap(~country, scales='free_y')+theme_bw()+labs(colour='Size class')+theme(legend.position=c(.68,.07))+
...
plot_lc_l <- ggplot(data=full_descr, aes(x=year, y=lc_l_mean, colour=as.factor(szclass)))+
  geom_line(size=1)+facet_wrap(~country, scales='free_y')+theme_bw()+labs(colour='Size class')+theme(legend.position=c(.68,.07))+
...
for (i in 1:length(unique(full_descr$country))){
  p <- ggplot(data=subset(full_descr, country==unique(full_descr$country)[i]), aes(x=year, y=lc_l_mean_norm, colour=as.factor(szclass)))+
    geom_line(size=1)+theme_bw()+labs(colour='Size class')+xlab('Year')+ylab('Mean labour cost per employee')+
    ggtitle(unique(full_descr$country)[i])
#  print(p)
  ggsave(filename=paste0(unique(full_descr$country)[i], '.pdf'),
         plot=p,
         path = graphs_dir,
         device='pdf',
...
# Unit labour cost
plot_ulc <- ggplot(full_descr, aes(x=year, y=ulc_mean, colour=as.factor(szclass)))+
  geom_line(size=1)+facet_wrap(~country, scales='free_y')+theme_bw()+labs(colour='Size class')+theme(legend.position=c(.68,.07))+
...
plot_safe <- ggplot(full_descr, aes(x=year, y=SAFE, colour=as.factor(szclass)))+
  geom_line(size=1)+facet_wrap(~country, scales='free_y')+theme_bw()+labs(colour='Size class')+theme(legend.position=c(.68,.07))+
...
plot_absconstr <- ggplot(full_descr, aes(x=year, y=absconstrained, colour=as.factor(szclass)))+
  geom_line(size=1)+facet_wrap(~country, scales='free_y')+theme_bw()+labs(colour='Size class')+theme(legend.position=c(.68,.07))+
...
# Financial gap
plot_fingap <- ggplot(full_descr, aes(x=year,y=financial_gap_mean, colour=as.factor(szclass)))+
  geom_line(size=1)+facet_wrap(~country, scales='free_y')+theme_bw()+labs(colour='Size class')+theme(legend.position=c(.68,.07))+
...
plot_mac <- ggplot(full_ind, aes(x=year, y=mac_rel_share, colour=as.factor(szclass), group=country))+
  geom_line(size=1)+facet_wrap(country~mac_sector, scales='free_y')+theme_bw()



# Sectoral, country average collateral levels
plot_sec_collateral_mean <- ggplot(data=full_ind %>%
...
plot_sec_ULC_mean <- ggplot(data=full_ind %>%
                                     group_by(year, mac_sector, szclass) %>% 
