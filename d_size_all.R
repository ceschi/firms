...
# diffing and sorting
new_info <- anti_join(full_descr_2012, full_descr, c('country', 'szclass', 'year'))
full_descr <- rbind(full_descr, new_info) %>% arrange(., country, szclass, year)

##### Selected vars ##### 
# First dataset, 5th release
d_size_all <- full_descr %>% 
  select(country, 
         szclass,       # size class
         year, 
...
full_descr <- merge(full_descr, full_descr %>% 
                    ...
                    by=c('country', 'year'))%>% 
  arrange(country, szclass, year)
full_descr$size_rel_share <- full_descr$l_count/full_descr$size_tot_count
...
full_descr <- merge(full_descr, full_descr %>% 
                    ...
                    by=c('country', 'year')) %>% 
  arrange(country, szclass,year)
full_descr$rel_emp_share <- full_descr$tot_l/full_descr$tot_emp
